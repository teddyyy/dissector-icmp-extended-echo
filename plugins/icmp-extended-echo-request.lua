...
f.checksum = ProtoField.new("Checksum", "icmp.ext.echo.req.checksum", ftypes.UINT8, nil, base.HEX)
f.identifier = ProtoField.new("Identifier", "icmp.ext.echo.req.identifier", ftypes.UINT8, nil, base.HEX)
f.sequence = ProtoField.new("Sequence Number", "icmp.ext.echo.req.seq", ftypes.UINT8, nil, base.DEC)
f.reserved = ProtoField.new("Reserved", "icmp.ext.echo.req.rsrvd", ftypes.UINT8, nil, base.DEC)
...
function icmp_ext_echo_req_proto.dissector(buffer, pinfo, tree)
	if icmpv6_type().value == 160 then
		...
		subtree:add(f.reserved, buffer(61, 1))
	end
end
